...
CRITICAL: Bell curve is trigger 18 — ABSOLUTE LAST resort. ONLY use it when the narration literally contains ONE of these exact phrases: "bell curve", "normal distribution", "Gaussian distribution". Do NOT use it for: "statistics", "average", "spread", "data", "sample", "population", "variance alone", "standard deviation alone". When in doubt, use fm_animate_glow_reveal or fm_animate_counter instead. The bell curve has caused more crashes and visual confusion than any other visual. Default AWAY from it.
CRITICAL: fm_animate_histogram ≠ fm_animate_bell_curve. Histogram = binned counts. Use for "frequency", "bins", "how data is spread".
CRITICAL: fm_formula lines must use ONLY standard ASCII characters that Text() can render. Do NOT use Unicode symbols like Σ, σ, μ, ∑, π, ∂, ∇ in fm_formula — they render as black boxes. Write them out as words or use ASCII approximations: "sum" instead of "Σ", "sigma" instead of "σ", "mu" instead of "μ", "p_i" instead of subscripts.
...
- NEVER unpack fm_* returns as 3 values: `tracker, lbl, cat = fm_animate_gauge(self, ...)` → CRASH. Always exactly 2: `result, _ = fm_animate_gauge(self, ...)`. Every fm_animate_* returns exactly (collected_vgroup, main_mob). This includes fm_animate_donut — it returns (collected, pct_lbl), NOT 3 values.

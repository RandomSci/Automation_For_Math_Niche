...
- fm_concept_pills() does NOT accept `accent_color` as a param — use `colors=[BRAND_GOLD, BRAND_GOLD, BRAND_GOLD]` to control color, or just omit it and let the function pick. Never pass accent_color= to fm_concept_pills.
- NEVER compare mob.get_color() with a BRAND_* hex string: `bar.get_color() == BRAND_GREEN` → CRASH TypeError (ManimColor vs str). get_color() returns a ManimColor object, not a string. Never use get_color() for conditional logic — track which objects have which color yourself using Python variables instead.
- NEVER index into fm_animate_bar_chart or fm_animate_comparison_bars return values to get individual bars: `chart_group[1:]` or `chart_group[1+3]` → IndexError. The returned VGroup is NOT a simple list of bars in a predictable order. If you need to animate individual bars, build the bars yourself with raw Manim Rectangle/RoundedRectangle, not via an fm_* function.

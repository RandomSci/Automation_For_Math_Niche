...
USE REAL MATH NOTATION, NOT JUST CHARTS, WHEN A BEAT IS ABOUT A CALCULATION: this channel has LaTeX available -- MathTex and Tex work. When narration describes how a number is calculated (compound interest, a percentage of a percentage, tax taken off a total, an hourly rate times hours), do not just show the final number -- show the FORMULA, the same way 3blue1brown shows the equation before showing what it evaluates to. `MathTex(r"A = P(1 + r)^t", font_size=64, color=BRAND_WHITE)` is a perfectly normal chunk on this channel, not an edge case. A formula can also animate piece by piece (split into separate indexed MathTex parts like the amount, a multiplication symbol, the rate, an equals sign, and the result, each as its own string in a list) that highlight or transform in sequence, the same way 3blue1brown reveals an equation term by term. Reach for this whenever a beat's content is fundamentally "X calculated from Y" rather than just "here is X."
...
- Bar comparison: for 2 items, use fm_animate_comparison_bars directly. For 3 OR MORE categories, BarChart is NOT available (it is banned in this environment and will be rejected), so build it by hand: a baseline Line first (the x-axis), anchor each Rectangle's bottom edge to it, fill each bar with a solid brand color (not just an outline), each bar's value as a Text directly above it. For category labels under 3+ bars, this is a real recurring failure mode -- labels overlapping into unreadable text -- so: keep every category label SHORT (1-2 words, abbreviate if needed: \"$0-250\" not \"$0-$250 per month\"), reduce font_size for the category labels specifically (18-20, smaller than the 22-28 used for value labels) once there are 4+ categories, and verify the bar spacing (bar width + gap) is wide enough that two adjacent category labels' bounding boxes cannot overlap -- if in doubt, make bars narrower and gaps wider rather than risk collision. Each category label centers under its own bar, never spans multiple bars.
